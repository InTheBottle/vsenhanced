...
void main()
{
    ...
    for( int i = 0; i < kernelSize; ++i)
    {
		...
		vec3 sampleNormal = normalize(texture(gNormal, offset.xy).rgb);
		float depthDiff = sampleDepth - (sample.z + bias);
		float normalWeight = smoothstep(0.25, 0.95, dot(sampleNormal, normal));
		float depthWeight = 1.0 - smoothstep(0.12, 0.24, abs(fragPos.z - sampleDepth));
		float rangeCheck = 0;
		...
		rangeCheck *= mix(1.0, normalWeight * depthWeight, 0.65);
	...
	// We need MOAR SSAO >:D
	if (!leavesHack) {
		float detailBoost = mix(1.2, 1.4, distanceFade);
		occ = 1 - (1-occ) * detailBoost;
	}
