...
float getFogAmountForSky(vec3 skyPosition, vec3 skyPosNorm, float sealevelOffsetFactor, float horizonFog) {
	...
	float density = invHorizonDistance * 0.68;

	float fac = max(0.0, density - 0.5) * max(min(1.0, 5.0 * fogAmount), horizonFog);
	fogAmount += fac;
...
void getSkyColorAt(vec3 skyPosition, vec3 sunPosition, float sealevelOffsetFactor, float skyLightIntensity, float horizonFog, out vec4 skyColor, out vec4 skyGlow)
{
	...
	vec4 noiseCol = NoiseFromPixelPosition(ivec2(gl_FragCoord.xy), 17, horizontalResolution) * 0.08;
	...
	float sunHeight = clamp((sunPos.y + 1.0) * 0.5, 0.0, 1.0);
	float horizonWarmth = 1.0 - smoothstep(0.28, 0.68, sunHeight);
	vec3 sunTint = mix(vec3(1.0, 0.86, 0.62), vec3(1.0, 0.55, 0.28), horizonWarmth);
	Kg.rgb = mix(Kg.rgb, Kg.rgb * sunTint, clamp(Kg.a + horizonWarmth * 0.35, 0.0, 1.0));
	...
	float horizonScatter = smoothstep(0.55, 1.0, invHorizonDistance) * (1.0 - fogAmount) * skyLightIntensity;
	vec3 scatterColor = mix(rgbaFog.rgb, Kg.rgb, clamp(Kg.a + sunsetMod * 0.35, 0.0, 1.0));
	skyColor.rgb = mix(skyColor.rgb, scatterColor, horizonScatter * 0.12);
	
	skyColor += noiseCol * (1.0 - fogAmount);
...
vec4 getSkyGlowAt(vec3 skyPosition, vec3 sunPosition, float sealevelOffsetFactor, float skyLightIntensity, float horizonFog, float proximityMul)
{
	...
	vec3 sunPos = sunPosition.xyz;
	vec4 noiseCol = NoiseFromPixelPosition(ivec2(gl_FragCoord.xy), 23, horizontalResolution) * 0.08;
	...
	float horizonScatter = smoothstep(0.55, 1.0, invHorizonDistance) * (1.0 - fogAmount) * skyLightIntensity;
	Kg.rgb = mix(Kg.rgb, rgbaFog.rgb, horizonScatter * 0.08);
	
	Kg += noiseCol * (1.0 - fogAmount);
